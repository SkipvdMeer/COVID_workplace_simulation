...
for (i in 1:length(x)){
  ...
  plot_data$PCR.frequency <- factor(plot_data$PCR.frequency, levels = c("workday", "semiweekly", "weekly", "monthly", "none"))
...
PCR.frequency = c("none", "workday", "semiweekly", "weekly", "monthly")
R = c(R)
...
# Order factors
data$PCR.frequency <- factor(data$PCR.frequency, levels = c("workday", "semiweekly", "weekly", "monthly", "none"))
...
# Filter on no PCR
df3 <- final_data %>% filter(PCR.frequency == 'none')

# Set Bubble size in right order for the plot
df3$Bubble.size <- factor(df3$Bubble.size, levels=c("5", "10", "25", "50", "none"))

# Create plot
ggplot(data=df3, aes(x = Network.size, y = Proportion.infected, color = Bubble.size)) +
  ...
  theme(legend.position="top", legend.title = element_text(size = 20),
        ...
        axis.title=element_text(size=12), strip.text = element_text(size=12.5),
        axis.title.x = element_text(size = 18), axis.title.y = element_text(size = 18)) +
  facet_wrap(vars(R), ncol=3) +
...
```{r, fig.height=12, fig.width=18}
# Filter on no Bubble 
df3 <- final_data %>% filter(Bubble.size == 'none')

# Set PCR frequency is right order for plot
df3$PCR.frequency <- factor(df3$PCR.frequency, levels=c("workday", "semiweekly", "weekly", "monthly", "none"))
